...
import cbp.shutter, cbp.monochromater
...
def get_keithley(rm, duration = 1, photons = 100000, charge = 10**-6, wavelength = 550, mode = 'curr', analysisType = 'duration', doSingle = False, doReset = True, photonFile = 'test.dat', doShutter = True):
...
def main(runtype = "keithley", duration = 1, photons = 100000, charge = 10**-6, wavelength = 550, mode = 'curr', analysisType = "duration", doSingle = False, doReset = False, photonFile = 'test.dat', doShutter = True):

    if runtype == "keithley":
        rm = visa.ResourceManager('@py')
        if doSingle:
            times, photos, totphotons = get_keithley(rm = rm, duration = duration, mode = mode, photons = photons, charge = charge, wavelength = wavelength, analysisType = analysisType, doSingle = doSingle, doReset = doReset, photonFile = photonFile, doShutter = doShutter)
            print times, photos, totphotons
            return times, photos, totphotons
        else:
            photo1, photo2 = get_keithley(rm = rm, duration = duration, mode = mode, photons = photons, charge = charge, wavelength = wavelength, analysisType = analysisType, doSingle = doSingle, doReset = doReset, photonFile = photonFile, doShutter = doShutter)
            print photo1, photo2 
...
if __name__ == "__main__":
    ...
    if opts.doKeithley:
        main(runtype = "keithley",duration = opts.duration, photons = opts.photons, charge = opts.charge, wavelength = opts.wavelength, mode = opts.mode, analysisType = opts.analysisType, doSingle = opts.doSingle, doReset = opts.doReset, photonFile = opts.photonFile, doShutter = opts.doShutter)
